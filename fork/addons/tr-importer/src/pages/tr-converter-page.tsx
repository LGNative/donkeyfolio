  ...
  const handleImport = React.useCallback(async () => {
    ...
    try {
      ...
      const saveWithBisect = async (
        ...
        const result = (await ctx.api.activities.saveMany({ creates: chunk })) as {
          created?: { id?: string }[];
          errors?: { action: string; message: string; id?: string }[];
        };
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <p className="text-muted-foreground text-xs">
              <strong className="text-foreground/80">Not a Trade Republic CSV?</strong> The{" "}
              <strong>AI Importer</strong> addon (by{" "}
              <a
                href="https://github.com/afadil/wealthfolio/tree/main/addons/wealthfolio-importer"
                target="_blank"
                rel="noreferrer"
              ...
              ) extracts transactions from any broker statement (PDF/image) using Claude or GPT —
