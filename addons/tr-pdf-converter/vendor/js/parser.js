...
function isCashStartLabel(text) {
  ...
  return normalized === 'UMSATZUBERSICHT' ||
...
function isCashEndLabel(text) {
  const normalized = normalizeHeaderText(text);
  return normalized.includes('BARMITTELUBERSICHT') ||
...
function isInterestStartLabel(text) {
  const normalized = normalizeHeaderText(text);
  return normalized === 'TRANSAKTIONSUBERSICHT' ||
    normalized === 'TRANSACTION OVERVIEW' ||
    normalized === 'APERCU DES TRANSACTIONS';
}

function isInterestEndLabel(text) {
  const normalized = normalizeHeaderText(text);
  return normalized.includes('HINWEISE ZUM KONTOAUSZUG') ||
    normalized.includes('NOTES TO ACCOUNT STATEMENT') ||
    normalized.includes('ACCOUNT STATEMENT NOTES') ||
    normalized.includes('NOTES RELATIVES AU RELEVE DE COMPTE');
}
...
async function parsePDF(pdf, options = {}) {
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    const cashStartMarker = items.find(item => {
      ...
      return isCashStartLabel(item.text) ||
    ...
    const interestStartMarker = items.find(item => {
      const normalized = normalizeHeaderText(item.text);
      return isInterestStartLabel(item.text) ||
...
function findCashHeaders(items) {
  const headerKeywords = [
    ...
    // French equivalents
    'ENTREE', 'SORTIE', 'ARGENT', 'SOLDE',
    // Spanish equivalents
    'FECHA', 'DESCRIPCION', 'ENTRADA', 'SALIDA', 'DINERO'
  ];
  ...
  const uppercaseItems = normalizedItems.filter(entry => {
  ...
  const matchAny = (labels) => {
    ...
    return potentialHeaders.find(p => normalizedLabels.includes(p._normalizedText || normalizeHeaderText(p.text))) || null;
  };
  ...
  const findCompositeHeader = (...parts) => {
    ...
    const firstCandidates = uppercaseItems.filter(p => (p._normalizedText || normalizeHeaderText(p.text)) === firstPart);
    for (const f of firstCandidates) {
      ...
      for (const part of remainingParts) {
        const nearby = uppercaseItems
          ...
          .filter(p => Math.abs(p.y - previous.y) <= 6 && p.x > previous.x - 2 && p.x < previous.x + previous.width + 120)
          .sort((a, b) => (Math.abs(a.y - previous.y) - Math.abs(b.y - previous.y)) || ((a.x - previous.x) - (b.x - previous.x)))[0];
  ...
  if (!headers.ZAHLUNGEN) {
    headers.ZAHLUNGSEINGANG =
      matchAny(['ZAHLUNGSEINGANG', 'IN ENTRATA', 'MONEY IN', "ENTRÉE D'ARGENT", "ENTREE D'ARGENT", 'ENTREE DARGENT', 'ENTRADA DE DINERO']) ||
    ...
    headers.ZAHLUNGSAUSGANG =
      matchAny(['ZAHLUNGSAUSGANG', 'IN USCITA', 'MONEY OUT', "SORTIE D'ARGENT", "SORTIE DARGENT", 'SALIDA DE DINERO']) ||
