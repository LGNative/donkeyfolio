...
function parseSecurities(pages) {
  ...
  const TAIL_WITH_DATE =
    /(\d{1,3}(?:[\.,]\d{3})*,\d{2})\s*(\d{2}\.\d{2}\.\d{4})\s*(\d{1,3}(?:[\.,]\d{3})*,\d{2})\s*$/;
  const ISIN = /\bISIN:\s*([A-Z]{2}[A-Z0-9]{10})\b/;
  const SKIP =
    /(POSITIONEN|STK\.?\s*\/\s*NOMINALE|KURS PRO ST[ÜU]CK|KURSWERT IN EUR|DEPOTAUSZUG|SEITE)/i;
...
function enrichTradingDataWithSecurities(tradingData, securities) {
  ...
  const totalCurrentValue = enrichedPnL.reduce(
    (sum, pos) => sum + (pos.currentValue || pos.costBasis),
    0,
  );
  const totalUnrealizedPnL = enrichedPnL.reduce((sum, pos) => sum + (pos.unrealizedPnL || 0), 0);
...
function parseTradingTransactions(cashTransactions) {
  ...
  cashTransactions.forEach((tx) => {
    ...
    // PATCH (Donkeyfolio): the original inline parser unconditionally stripped
    // every "." from the string and treated "," as the decimal separator. That
    // assumed German format and inflated US-format amounts ("€30.00" → "€3000"
    // → 3000) by 100×. Delegate to parseCurrency (exported on window by
    // parser.js) which auto-detects EU vs US format.
    const _parseAmt =
    ...
    if (trade.isBuy && tx.outgoing) {
      amount = _parseAmt(tx.outgoing) || 0;
    } else if (!trade.isBuy && tx.incoming) {
      amount = _parseAmt(tx.incoming) || 0;
    }
  ...
  return tradingTxs.sort(
    (a, b) => new Date(parseGermanDate(a.date)) - new Date(parseGermanDate(b.date)),
  );
}
...
function createTradingCharts(tradingData, tradingTransactions) {
  ...
  if (!tradingData || tradingData.pnlSummary.length === 0) {
    container.innerHTML =
      '<p class="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">Keine Trading-Daten gefunden.</p>';
    return container;
  }

  // P&L per Stock Chart
  const pnlChartBox = document.createElement("div");
  pnlChartBox.className =
  ...
  if (tradingTransactions.length > 3) {
    ...
    timelineChartBox.className =
      "flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:col-span-2";
...
function renderTradingCharts(tradingData, tradingTransactions) {
  setTimeout(() => {
    try {
      ...
      const pnlLabels = topPositions.map((pos) =>
        pos.stockName.length > 20 ? pos.stockName.substring(0, 17) + "..." : pos.stockName,
      );
      const pnlData = topPositions.map((pos) =>
        pos.isOpen ? -pos.costBasis : pos.realizedGainLoss,
      );
      const pnlColors = topPositions.map((pos) =>
        pos.isOpen ? "#6c757d" : pos.realizedGainLoss >= 0 ? "#10b981" : "#ef4444",
      );
...
function createTradingStatsSummary(tradingData) {
  ...
  const cards = [
    ...
    createCard(
      ...
      formatCurrency(tradingData.totalRealized),
      valueTone(tradingData.totalRealized),
    ),
    createCard(
      "arrow-down-up",
      "Netto Cash Flow:",
      formatCurrency(tradingData.totalNetCashFlow),
      valueTone(tradingData.totalNetCashFlow),
    ),
  ...
  if (tradingData.hasSecuritiesData) {
    cards.push(
      createCard(
        ...
        formatCurrency(tradingData.totalUnrealizedPnL),
        valueTone(tradingData.totalUnrealizedPnL),
      ),
      createCard(
        "pie-chart",
        "Aktueller Portfolio-Wert:",
        formatCurrency(tradingData.totalCurrentValue),
      ),
      createCard(
        "target",
        "Gesamt P&L:",
        formatCurrency(tradingData.totalPnL),
        valueTone(tradingData.totalPnL),
      ),
    );
  } else {
    cards.push(
          ...
          try {
            ...
            if (window.currentTradingData) {
              ...
              const activeTab = document.querySelector(
                '[data-tab-role="navigation"][data-active="true"]',
              );
              if (activeTab && activeTab.textContent.includes("Trading")) {
                // Find the corresponding content div
                const activeContent = document.querySelector(
                  '[data-tab-role="panel"][data-active="true"]',
                );
                if (activeContent) {
                  // Regenerate the trading component with enriched data
                  const enrichedComponent = renderTradingComponent(
                    window.currentTradingData,
                    window.currentTradingTransactions,
                  );
...
function clearSecuritiesData() {
  ...
  if (window.currentTradingData) {
    ...
    if (activeTab && activeTab.textContent.includes("Trading")) {
      ...
      if (activeContent) {
        const refreshedComponent = renderTradingComponent(
          window.currentTradingData,
          window.currentTradingTransactions,
        );
        activeContent.innerHTML = "";
...
function renderTradingTab() {
  const tradingComponent = renderTradingComponent(
    window.currentTradingData,
    window.currentTradingTransactions,
  );
