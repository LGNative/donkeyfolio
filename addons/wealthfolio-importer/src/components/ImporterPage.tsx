    ...
    try {
      ...
      if (file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf")) {
        ...
        if (
          ...
          !confirm(
            `This document has ${pages.length} pages.\n\n` +
              `Processing may take several minutes and consume significant API credits ` +
              `(~$0.10-0.50 per 50 pages on Anthropic, similar on OpenAI).\n\n` +
              `If this is a Trade Republic statement, the dedicated TR PDF Converter ` +
              `addon is much faster (~10s) and free.\n\n` +
              `Continue with AI extraction anyway?`,
          )
        ) {
          setStep("upload");
          return;
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (invalid.length > 0) {
        ...
        const symbolList =
          unresolvedSymbols.length > 0 ? unresolvedSymbols.join(", ") : "unknown symbols";
        const message =
          valid.length > 0
      ...
      }>("/api/v1/activities/import", { accountId: selectedAccount, activities: toImport });
  ...
  return (
    ...
    >
      ...
      {step === "extracting" && (
        ...
        >
          ...
          <p style={{ marginTop: "12px" }}>
            Extracting transactions from <strong>{fileName}</strong>...
          </p>
      ...
      {step === "done" && (
        ...
        >
          <p style={{ fontSize: "14px", fontWeight: 500, color: "hsl(142 71% 45%)" }}>
            {importResult}
          </p>
          <button
            onClick={startOver}
