...
export function evaluateConfidence(txn: ExtractedTransaction): FieldFlag[] {
  ...
  if (!txn.symbol) flags.push({ field: "symbol", reason: "Missing symbol" });
  if (!txn.date) flags.push({ field: "date", reason: "Missing date" });
...
export async function extractTransactions(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      throw new Error(
        `Failed processing ${chunkPageRange(chunks[i])} (chunk ${i + 1} of ${chunks.length}): ${msg}`,
      );
    }
  }
  return allResults;
...
export function parseResponse(text: string | undefined | null): ExtractedTransaction[] {
  if (!text)
    throw new Error(
      "Empty response from AI provider. The document may be unreadable — try a clearer scan or image.",
    );
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Could not parse AI response as JSON. Please try again with a clearer document.",
      );
    }
  }
...
export function validateTransaction(t: unknown): ExtractedTransaction {
  ...
  return {
    ...
    quantity:
      typeof obj.quantity === "number" && isFinite(obj.quantity) ? Math.max(0, obj.quantity) : 0,
    activityType: (ACTIVITY_TYPES as readonly string[]).includes(obj.activityType as string)
      ...
      : "BUY",
    unitPrice:
      typeof obj.unitPrice === "number" && isFinite(obj.unitPrice) ? Math.max(0, obj.unitPrice) : 0,
    currency:
      typeof obj.currency === "string" && CURRENCY_RE.test(obj.currency) ? obj.currency : "USD",
